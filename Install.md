...
Building and installing ToxClient
-----------------------------
...
After you installed the dependencies, clone, build and install ValaTox:

    git clone --recursive git://github.com/ValaTox/client.git ValaTox
    cd ValaTox
    mkdir build
    cd build
    # For glib >= 2.34
...
###Compiling ToxClient
    ...
    git clone git://github.com/ToxClient/client.git ToxClient
    cd ToxClient
    mkdir build
    cd build
...
then cmake can't find one or more dependencies needed to build ToxClient.
